...
interface AudioDevice {
  deviceId: string;
  label: string;
  key: string; // Added to ensure unique keys
}
...
export default function Player({
    ...
    const fetchDevices = async () => {
      try {
        ...
        const outputDevices = devices
          ...
          .map((device, index) => ({
            deviceId: device.deviceId || `default-${index}`, // Ensure unique deviceId
            label: device.label || `Device ${index + 1}`,
            key: device.deviceId ? `device-${device.deviceId}` : `default-${index}`, // Unique key
          }));

        // Add default device only if not already present
        setAudioDevices([
  ...
  const handleDeviceChange = async (deviceId: string) => {
    ...
    if (audioRef.current) {
      try {
        ...
        setShowDevicePanel(false); // Close panel after selection
      } catch (err) {
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 bg-[#0E0E0E] p-4 z-30 text-white shadow-lg transition-all ${
        isFullScreen ? "top-0 h-screen flex flex-col justify-center" : ""
      } ${focusMode ? "bg-opacity-80 backdrop-blur-md" : ""}`}
    >
      <div className="flex items-center justify-between max-w-7xl mx-auto flex-col md:flex-row gap-4" {...swipeHandlers}>
        {/* Song Info */}
        ...
        <div className="flex items-center space-x-4 w-full md:w-1/3 justify-end">
          ...
          >
            <Rows3 className="w-6 h-8 cursor-pointer"/>
          </motion.button>
          ...
          >
            <span className="text-xs">{focusMode ? "Normal" : "Focus"}</span>
          </motion.button>
        </div>
      </div>
